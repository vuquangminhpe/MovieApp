...
export default function KeyWordsMovie_TV_All() {
  ...
  const {
    ...
    queryKey: ['dataDiscover_Movie', KeyWordsID],
    queryFn: async ({ pageParam = 1 }) => {
      const result = DiscoverApi.getListDiscover_Movie({
        page: pageParam,
        language: 'en-US',
  ...
  const { data: dataDiscover_TV } = useQuery({
    queryKey: ['dataDiscover_TV', KeyWordsID],
    queryFn: () => DiscoverApi.getListDiscoverTV({ with_keywords: String(KeyWordsID) })
  })
  const countMovies = useMemo(
    () =>
      dataDiscover_Movie?.pages.flatMap(
        (page) => page.data.total_results as MovieTrendings | readonly MovieTrendings[]
      ) ?? [],
    [dataDiscover_Movie]
  )
  const allMovies = useMemo(
    () =>
      dataDiscover_Movie?.pages.flatMap((page) => page.data.results as MovieTrendings | readonly MovieTrendings[]) ??
      [],
    [dataDiscover_Movie]
  )
      ...
      <div className='w-full'>
        ...
        <div className='flex -translate-y-16  justify-between mx-5'>
          ...
          <div className='text-gray-400 font-bold text-xl'>
            {countMovies as unknown as number} {' movies'}
          </div>
        </div>

        <div className='mt-6 px-12'>
          {allMovies?.map((itemMovie: MovieTrendings) => (
            <Link
