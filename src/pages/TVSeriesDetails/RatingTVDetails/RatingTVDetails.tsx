...
export default function RatingTVDetails({
  idTV,
  dataTV,
  min = 0,
  max = 100,
  ...
  const { data: dataAccount_States, refetch } = useQuery({
    queryKey: ['account_states', idTV],
    queryFn: () => TVSeriesApi.GetAccountStates(idTV)
  })
  const deletedMutation = useMutation({
    mutationFn: () => TVSeriesApi.DeletedRatingTV(idTV)
  })
  ...
  const ratingMovieMutation = useMutation({
    mutationFn: () => TVSeriesApi.AddRatingTV(idTV, value / 10)
  })
          ...
          <DrawerHeader>
            ...
            <DrawerDescription className='text-black'>
              <div className='flex justify-between items-center mb-2'>
                <div>what did you think of {dataTV?.original_name}?</div>
